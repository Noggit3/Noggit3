...
void MapView::createGUI()
{
  ...
  _terrain_tool_dock = new QDockWidget("高度", this);
...
#define ADD_TOGGLE_NS(menu_, name_, property_)                    \
  ...
  ADD_ACTION (file_menu, "保存当前大块", "Ctrl+Shift+S", [this] { save(save_mode::current); });
  ADD_ACTION (file_menu, "保存已修改大块", QKeySequence::Save, [this] { save(save_mode::changed); });
  ADD_ACTION (file_menu, "保存地图", "Ctrl+Shift+A", [this] { save(save_mode::all); });
  ...
  ADD_ACTION_NS (file_menu, "下次开启时强制uid检查", [this] { _force_uid_check = true; });
  file_menu->addSeparator();


  edit_menu->addSeparator();
  edit_menu->addAction(createTextSeparator("选中的目标物"));
  edit_menu->addSeparator();
  ADD_ACTION (edit_menu, "删除", Qt::Key_Delete, [this] { DeleteSelectedObject(); });
  ADD_ACTION (edit_menu, "重置旋转值", "Ctrl+R", [this] { ResetSelectedObjectRotation(); });
  ADD_ACTION (edit_menu, "放地面", Qt::Key_PageDown, [this] { snap_selected_models_to_the_ground(); });


  edit_menu->addSeparator();
  edit_menu->addAction(createTextSeparator("选项"));
  edit_menu->addSeparator();
  ADD_TOGGLE_NS (edit_menu, "锁光标模式", _locked_cursor_mode);


  assist_menu->addSeparator();
  assist_menu->addAction(createTextSeparator("模型"));
  assist_menu->addSeparator();
  ADD_ACTION (assist_menu, "查看最近导入的M2", "Shift+V", [this] { objectEditor->import_last_model_from_wmv(eEntry_Model); });
  ADD_ACTION (assist_menu, "查看最近导入的WMO", "Alt+V", [this] { objectEditor->import_last_model_from_wmv(eEntry_WMO); });
  ADD_ACTION_NS (assist_menu, "模型助手", [this] { objectEditor->helper_models_widget->show(); });

  assist_menu->addSeparator();
  assist_menu->addAction(createTextSeparator("当前ADT"));
  assist_menu->addSeparator();
  ...
  assist_menu->addSeparator();
  assist_menu->addAction(createTextSeparator("已加载ADT"));
  assist_menu->addSeparator();
  ADD_ACTION_NS ( assist_menu
                , "修复隔断"
                , [this]
  ...
  assist_menu->addSeparator();
  assist_menu->addAction(createTextSeparator("其他"));
  assist_menu->addSeparator();
  ADD_ACTION_NS ( assist_menu
                , "非首层纹理大透明度"
                , [this]
  ...
  view_menu->addAction(createTextSeparator("显示"));
  view_menu->addSeparator();
  ADD_TOGGLE (view_menu, "物件", Qt::Key_F1, _draw_models);
  ADD_TOGGLE (view_menu, "WMO物件", Qt::Key_F2, _draw_wmo_doodads);
  ADD_TOGGLE (view_menu, "地面", Qt::Key_F3, _draw_terrain);
  ADD_TOGGLE (view_menu, "水", Qt::Key_F4, _draw_water);
  ADD_TOGGLE (view_menu, "WMOs", Qt::Key_F6, _draw_wmo);
  ADD_TOGGLE (view_menu, "网格线", Qt::Key_F7, _draw_lines);
  ADD_TOGGLE (view_menu, "等高线", Qt::Key_F9, _draw_contour);
  ADD_TOGGLE (view_menu, "顶点线", Qt::Key_F10, _draw_wireframe);
  ADD_TOGGLE (view_menu, "模型动画", Qt::Key_F11, _draw_model_animations);
  ADD_TOGGLE (view_menu, "雾", Qt::Key_F12, _draw_fog);
  ADD_TOGGLE_NS (view_menu, "飞行边界", _draw_mfbo);
  ADD_TOGGLE (view_menu, "孔线", "Shift+F7", _draw_hole_lines);
  ADD_TOGGLE_NS (view_menu, "模型包围盒", _draw_models_with_box);
  //! \todo space+h in object mode
  ADD_TOGGLE_NS (view_menu, "隐藏的模型", _draw_hidden_models);

  view_menu->addSeparator();
  view_menu->addAction(createTextSeparator("ADT预览图"));
  view_menu->addSeparator();

  ADD_TOGGLE (view_menu, "显示", Qt::Key_M, _show_minimap_window);
  ...
  ADD_TOGGLE_NS(view_menu, "显示ADT边界", _show_minimap_borders);
  ...
  ADD_TOGGLE_NS(view_menu, "显示光照区", _show_minimap_skies);
  ...
  view_menu->addSeparator();
  view_menu->addAction(createTextSeparator("窗口"));
  view_menu->addSeparator();
  ...
  ADD_ACTION(view_menu, "切换界面", Qt::Key_Tab, hide_widgets);


  ADD_TOGGLE (view_menu, "详细信息", Qt::Key_F8, _show_detail_info_window);
  ...
  ADD_TOGGLE (view_menu, "光标切换", "Ctrl+Alt+C", _show_cursor_switcher_window);
  ...
  ADD_TOGGLE (view_menu, "纹理调色", Qt::Key_X, _show_texture_palette_window);
  ...
  ADD_TOGGLE_NS(view_menu, "小纹理调色", _show_texture_palette_small_window);
  ...
  ADD_TOGGLE (help_menu, "快捷键", "Ctrl+F1", _show_keybindings_window);
                ...
                , []
                  {
                    ShellExecute ( nullptr
                                 , "open"
                                 , "https://jq.qq.com/?_wv=1027&k=5I6O0TG"
                                 , nullptr
                                 , nullptr
                                 , SW_SHOWNORMAL
                                 );
  ...
  ADD_ACTION (view_menu, "快进", Qt::Key_N, [this] { mTimespeed += 90.0f; });
  ADD_ACTION (view_menu, "慢放", Qt::Key_B, [this] { mTimespeed = std::max (0.0f, mTimespeed - 90.0f); });
  ADD_ACTION (view_menu, "暂停", Qt::Key_J, [this] { mTimespeed = 0.0f; });
  ...
  ADD_ACTION (view_menu, "视角速度-", Qt::Key_O, [this] { _camera.move_speed *= 0.5f; });
  ADD_ACTION (view_menu, "视角速度+", Qt::Key_P, [this] { _camera.move_speed *= 2.0f; });

  ADD_ACTION (file_menu, "保存小地图", "Ctrl+Shift+P", [this] { Saving = true; });
...
MapView::MapView( math::degrees camera_yaw0
  ...
  , _minimap_dock (new QDockWidget ("ADT预览图", this))
  , _texture_palette_dock(new QDockWidget(this))
{
  ...
  QStatusBar *statusBar = _main_window->statusBar();
  statusBar->addWidget(_status_position);
  statusBar->addWidget(_status_selection);
  statusBar->addWidget(_status_area);
  statusBar->addWidget(_status_time);
  statusBar->addWidget(_status_fps);
  ...
  connect ( this
    ...
    [=]
    {
      try
      {
          ...
          statusBar->removeWidget(_status_position);
          statusBar->removeWidget (_status_selection);
          statusBar->removeWidget (_status_area);
          statusBar->removeWidget (_status_time);
          statusBar->removeWidget (_status_fps);
      }
      catch(...)
      {
          throw;
