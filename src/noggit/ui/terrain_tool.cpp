...
namespace noggit
{
  namespace ui
  {
      ...
      , _vertex_mode(eVertexMode_Center)
    {
      ...
      QRadioButton* radio_polynomial = new QRadioButton ("多项式");
      QRadioButton* radio_trigo = new QRadioButton ("三角函数");
      QRadioButton* radio_quadra = new QRadioButton ("平方");
      QRadioButton* radio_gauss = new QRadioButton ("高斯分布");
      QRadioButton* radio_vertex = new QRadioButton ("*顶点");
      ...
      QGroupBox* radius_group (new QGroupBox ("半径"));
      QFormLayout* radius_layout (new QFormLayout (radius_group));
      radius_layout->addRow ("外层:", _radius_spin);
      radius_layout->addRow (_radius_slider);
      radius_layout->addRow ("内层:", _inner_radius_spin);
