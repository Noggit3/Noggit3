...
namespace noggit
{
  namespace ui
  {
      ...
      , _null_widget (new QWidget (this))
    {
      ...
      auto settings_action (file_menu->addAction ("设置"));
      ...
      auto about_action (file_menu->addAction ("关于"));
      ...
      auto cloasemap_action (file_menu->addAction ("关闭地图"));
      QObject::connect ( cloasemap_action, &QAction::triggered
          , [this]
      {
          rebuild_menu();
      }
      ...
      auto mapmenu_action (file_menu->addAction ("退出"));
    ...
    void main_window::loadMap(int mapID)
    {
      ...
      LogError << "地图 " << mapID << " 未找到 " << std::endl;
    }
    ...
    void main_window::build_menu()
    {
      ...
      entry_points_tabs->addTab (continents_table, "大陆");
      entry_points_tabs->addTab (dungeons_table, "地下城");
      entry_points_tabs->addTab (raids_table, "团本");
      entry_points_tabs->addTab (battlegrounds_table, "战场");
      entry_points_tabs->addTab (arenas_table, "竞技场");
      entry_points_tabs->addTab (bookmarks_table, "书签");
    ...
    void main_window::prompt_exit(QCloseEvent* event)
    {
      ...
      prompt.setText ("是否退出?");
      prompt.setInformativeText ("未保存内容将丢失");
      prompt.addButton ("退出", QMessageBox::DestructiveRole);
      prompt.setDefaultButton (prompt.addButton ("取消", QMessageBox::RejectRole));
